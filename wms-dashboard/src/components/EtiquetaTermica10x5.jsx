...
function ConteudoEtiquetaTermica({ sku, descricao, codigoBarras, numeroSerie, etiquetaCodigo, enderecoSugerido }) {
    ...
    // gerado no recebimento - ver numero_serie_recebimento_seq); so
    // adiciona o "#" aqui quando o codigo exibido nao tiver um ainda
    // (caso caia no fallback etiquetaCodigo, que nao tem prefixo) -
    // sem isso ficava "##500576" na etiqueta impressa.
    const codigoComPrefixo = codigoExibido
        ? (String(codigoExibido).startsWith('#') ? codigoExibido : `#${codigoExibido}`)
        : null;
    return (
